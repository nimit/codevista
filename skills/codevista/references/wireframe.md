...
content.** Set the **body of a `wireframe` fence** to a self-contained,
semantic HTML fragment of the screen, and set the `surface=` attribute on the
fence. The renderer owns the surface footprint/aspect, the dark/light theme,
the hand-drawn font, and the rough.js sketch overlay — you never write
`<html>`/`<body>`/`<script>`/`<style>` tags or any width/height/coordinates.
You write real HTML layout and real product content; the renderer styles and
roughens it.

**A wireframe is an HTML screen plus a surface, authored as a `wireframe`
fence:**

```wireframe surface=browser
...
wireframe frame, root container, `.wf-card`, or `.wf-box`.
...
the plan. Put those in a separate `diagram` or the document
...
**Loading / skeleton states.** Set `skeleton=true` on the `wireframe` fence and
...
skeleton register automatically. Never fake a loader with raw HTML
...
state name belongs — never encode Before/After inside the html.
